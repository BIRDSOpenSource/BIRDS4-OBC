...
Void Send_Data_Packets(unsigned int32 start_address, unsigned int32 data_size, unsigned int32 packets,int1 x)
{
 ...
 unsigned int32 num_of_pckt;
    ...
    if(packets == 0)                                                             //si no recibe nro de packets convierte size a nro de packets
    {
...
{ 
   ...
   //========================= 22 ---- 102 ====================================
     if(x==1)
     {
        for (int32 ele_num = 22; ele_num <= 102; ele_num++)                      //dependiendo del num del packt va cambiando start address
        {      
...
unsigned int32 next_size = packet_set_size * (packet_set_num - 1);               //para saber que tamaño de datos queda
